...
mod embedded {
    ...
    embed_migrations!("sync_migrations");
}
...
impl PostgresConfig {
    fn from_env() -> Self {
        Self {
            host: env::var("SYNC_DATABASE_HOST").expect("SYNC_DATABASE_HOST must be set"),
            user: env::var("SYNC_DATABASE_USER").expect("SYNC_DATABASE_USER must be set"),
            password: env::var("SYNC_DATABASE_PASSWORD").expect("SYNC_DATABASE_PASSWORD must be set"),
            name: env::var("SYNC_DATABASE_NAME").expect("SYNC_DATABASE_NAME must be set"),
            pool_max: env::var("SYNC_DATABASE_POOL_MAX")
                .expect("SYNC_DATABASE_POOL_MAX must be set")
                .parse::<usize>()
                .expect("SYNC_DATABASE_POOL_MAX is usize"),
            port: env::var("SYNC_DATABASE_PORT")
                .expect("SYNC_DATABASE_PORT must be set")
                .parse::<u16>()
                .expect("SYNC_DATABASE_PORT is u16"),
        }
    }
}
impl From<PostgresConfig> for tokio_postgres::Config {
    fn from(_: PostgresConfig) -> Self {
        ...
        pg_config.port(postgres_config.port);
        pg_config
    }
}
