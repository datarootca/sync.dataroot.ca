...
pub async fn execute(
    ...
    article_create_models: Vec<ArticleCreateModel>,
) -> Result<Vec<ArticleModel>, DomainError> {
    let articles = article_repository.insert_many(article_create_models).await?;
    Ok(articles)
}
   ...
   use crate::api::lib::BatchOperations;
    ...
    async fn article_created() {
        ...
        let result = execute(
            ...
            vec![ArticleCreateModel::mock_default()],
        )
        .await;
