...
pub async fn execute(
    ...
    extids: Vec<String>,
) -> Result<Option<Vec<CityModel>>, DomainError> {
    if let Some(items) = city_repository.find_by_extids(extids).await? {
...
mod tests {
    ...
    use crate::{domain::city::model::{CityCreateModel, CityUpdateModel}, api::utils::random_string};
    ...
    async fn it_should_return_city_finded() {
        ...
        city_repository
            ...
            .return_once(|_| Ok(Some(vec![CityModel::mock_default()])));

        let result = execute(Arc::new(city_repository), vec![random_string(5)]).await;
    ...
    #[tokio::test]
    async fn it_should_return_error_no_content_cities() {
        let mut city_repository = MockFakeCityRepository::new();
