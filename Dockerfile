...
RUN cargo build --release --target $CARGO_BUILD_TARGET --bin sync
RUN strip target/$CARGO_BUILD_TARGET/release/sync
...
USER rust:rust
COPY --from=builder /app/target/$CARGO_BUILD_TARGET/release/sync /usr/local/bin
ENTRYPOINT ["sync"]